...
github link: https://github.com/open-ephys/analysis-tools/blob/master/OpenEphys.py
...
def loadFolder(folderpath, **kwargs):
    ...
    for i, f in enumerate(filelist):
        if ".continuous" in f:
            data[f.replace(".continuous", "")] = loadContinuous(
    ...
    with open(filepath, "rb") as f:
        ...
        while f.tell() < fileLength and records_read < n_records_to_read:
            ...
            if N != header["blockLength"]:
                raise IOError("Found corrupted record in block " + str(records_read))
            ...
            if np.any(record_marker != spec_record_marker):
                raise IOError("corrupted record marker at record %d" % records_read)
...
def loadSpikes(filepath):
    ...
    if float(header[" version"]) < 0.4:
        raise Exception(
            "Loader is only compatible with .spikes files with version 0.4 or higher"
        )
...
def readHeader(f):
    ...
    # by semicolons.
    header_string = (
        f.read(1024).decode("utf-8").replace("\n", "").replace("header.", "")
    )
        ...
        json.dump(
            {
                ...
                "refs": {"channels": [-1] * mapping.size},
                "recording": {"channels": [False] * mapping.size},
            },
            outfile,
            indent=4,
            separators=(",", ": "),
...
def get_filelist(folderpath, source="100", channels="all", recording=None):
    ...
    if recording is None or recording == 1:
        # The first recording has no suffix
        filelist = ["%s_CH%d.continuous" % (source, chan) for chan in channels]
    else:
        filelist = [
            "%s_CH%d_%d.continuous" % (source, chan, recording) for chan in channels
        ]
