import numpy as np
from scipy.signal import get_window,hilbert,find_peaks
...
def ripple_envelop_features(env_s, ripple_segment, ripple_segment_filt, fs, start_ind, end_ind, plot=False):
    """
    ...
    if plot:
        ...
        t_env_shifted = t_full[start_ind] + t_env  # Align envelope with full signal time
        ...
        axs[0].axvspan(t_full[start_ind], t_full[end_ind - 1], color='yellow', alpha=0.3, label="Ripple region")
        ...
        # -----------------------------------------------------
        axs[1].plot(t_full, ripple_segment_filt, color='gray', lw=1, alpha=0.6, label="Filtered Ripple")
        axs[1].plot(t_env_shifted, env_s, color='blue', lw=2, label="Envelope")

        # ---- Mark key envelope features ----
        axs[1].axvline(t_env_shifted[peak_idx], color='magenta', linestyle='--', lw=1.5, label='Peak')

        # Rise (20%→80%)
        if rise_start_idx is not None and rise_end_idx is not None:
            axs[1].axvline(t_env_shifted[rise_start_idx], color='green', linestyle='--', lw=1)
            axs[1].axvline(t_env_shifted[rise_end_idx], color='lime', linestyle='--', lw=1)

        # Fall (80%→20%)
        if fall_start_idx is not None and fall_end_idx is not None:
            axs[1].axvline(t_env_shifted[fall_start_idx], color='orange', linestyle='--', lw=1)
            axs[1].axvline(t_env_shifted[fall_end_idx], color='red', linestyle='--', lw=1)
        ...
        axs[1].text(0.02, 0.95, text, transform=axs[1].transAxes,
...
def ripple_time_domain_features(ripple_segment, ripple_segment_filt, fs, start_ind, end_ind, plot=False):
    """
    Compute:
    ...
    zero_crossing_rate = np.sum(np.abs(np.diff(np.sign(signal_mid)))) / (2 * len(signal_mid))
    ...
    IP_full, IF_full, IA_full = emd.spectra.frequency_transform(ripple_segment_filt, fs, 'hilbert')
    IP_ripple = IP_full[start_ind:end_ind]

    cycle_vect_good = emd.cycles.get_cycle_vector(IP_ripple, return_good=True,phase_edge=np.pi / 4,)
    cycle_vect_all = emd.cycles.get_cycle_vector(IP_ripple, return_good=False,phase_edge=np.pi / 4,)
    ...
    if plot:
        ...
        axs[0].axvspan(t_full[start_ind], t_full[end_ind - 1], color='yellow', alpha=0.2)
        ...
        for cid in unique_good_ids:
            ...
            axs[1].axvspan(start_t, end_t, color='green', alpha=0.4,
                           label='Good Cycle' if cid == list(unique_good_ids)[0] else "")

        axs[1].set_title("Filtered Ripple with All Cycles and Good Cycles")
        axs[1].axvspan(t_full[start_ind], t_full[end_ind - 1], color='yellow', alpha=0.2)
        axs[1].legend()
