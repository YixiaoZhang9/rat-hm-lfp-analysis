...
class SignalPlotViewer(QWidget):
    ...
    def update_plot(self):
        ...
        for i, ch in enumerate(self.channel_names):
            ...
            if "scoring" in ch.lower():
                ...
                x_step[:-1] = t_window
                x_step[-1] = t_window[-1] + (1 / self.fs)

                self.curves[i].setData(x_step, scaled, stepMode="center")
    ...
    def zoom_in_time(self):
        new_len = max(int(self.win_len * 0.8), int(self.fs))
        self.win_len = new_len
        self.slider.setMaximum(self.total_len - new_len)
        self.update_plot()

    def zoom_out_time(self):
        new_len = min(int(self.win_len * 1.25), self.total_len)
        self.win_len = new_len
        self.slider.setMaximum(self.total_len - new_len)
        self.update_plot()
    ...
    def update_event_regions(self):
        ...
        if not hasattr(self, "event_intervals") or not self.event_intervals:
            return

        win_start = self.start_idx
        win_end = self.start_idx + self.win_len
