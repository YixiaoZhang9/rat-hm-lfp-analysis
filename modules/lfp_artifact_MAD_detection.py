"""
Artifact detection based on MAD thresholding.

Adapted from:
Spyglass LFP artifact detection pipeline
Developed by Loren Frank Lab
https://github.com/LorenFrankLab/spyglass

Modifications:
- change it suitable for one-channel input
...
def mad_artifact_detector(
    ...
    **kwargs,
    ...
    # generate timestamps
    timestamps = np.arange(len(lfp)) / sampling_frequency

    mad = median_abs_deviation(lfp, nan_policy="omit", scale="normal")
    mad = 1.0 if (np.isclose(mad, 0.0) or ~np.isfinite(mad)) else mad

    mad_scaled = np.abs(lfp - np.nanmedian(lfp)) / mad
    is_artifact = mad_scaled > mad_thresh
