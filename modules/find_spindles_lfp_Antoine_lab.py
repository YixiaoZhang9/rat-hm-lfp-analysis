import numpy as np
from scipy.signal import detrend, resample_poly, firwin, filtfilt,find_peaks,welch,hilbert, get_window
import pywt
...
def spindle_detection_wavelet(raw_signal,signal, fs_raw, thrL = 1, thrH = 3):
    ...
    (valid_times, artifact_intervals_s) = mad_artifact_detector(raw_signal, mad_thresh = 6.0, proportion_above_thresh = 0.1,
                                                                removal_window_ms = 100.0,sampling_frequency = fs_raw)
    ...
    valid_mask_raw = intervals_to_mask(
        valid_times,
        t_raw
    )
    ...
    for start, end in zip(refined_s, refined_e):
        ...
        f, pxx = welch(seg2, fs=fs, window='hann', nperseg=len(seg2),
                       nfft=nfft_val, noverlap=0, detrend=False)
    ...
    for start_idx, peak_idx, end_idx in spindles:
        ...
        peak_frequency_spindle, mean_frequency_spindle = compute_peak_mean_frequency(signal_segment, fs_raw,freq_band=[9,20])
