...
def find_path_by_suffix(base_path, suffix, search_dir=True, use_regex=False):
    ...
    for root, dirs, files in os.walk(base_path):
        ...
        for name in targets:
            if (use_regex and pattern.search(name)) or (
                not use_regex and name.endswith(suffix)
            ):
    ...
    if len(matched_paths) == 0:
        ...
        raise ValueError(
            f"Multiple matching paths found ({len(matched_paths)}), expected only one."
        )
    else:
...
def read_recording(matched_dir, pattern):
    ...
    str_matched_file = find_path_by_suffix(
...
def get_folder_selected_channel_r5_8_20221005(
    csv_path, rat_number, target_region, idx_trial
):
    """
    ...
    Target_row = df[
        (df["Rat_Number"] == "Rat " + str(rat_number))
        & (df["Trial_Suffix"] == idx_trial)
    ]
