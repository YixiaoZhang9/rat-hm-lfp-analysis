#!/usr/bin/env python3
"""
Spindle Alignment Inspector — simplified AR-only view.

Shows the raw LFP, the 10-15 Hz band-passed LFP, and the AR R-value trace
that the detector (find_spindles_lfp) would compute for the same window.

The R trace is computed on the SAME 128 Hz downsampled signal that
find_spindles_lfp uses, so the R values it draws are numerically identical to
the detector's internal r_timeseries -- including the zeros where the AR
model placed no pole in the 10-15 Hz band.
"""
...
# --------------------------------------------------------------------------- #
# Configuration -- mirrors find_spindles_lfp defaults exactly
# --------------------------------------------------------------------------- #
...
# Detector parameters -- must match the call in extract_spindles.py
AR_TARGET_FS = 128
AR_ORDER = 8
...
# --------------------------------------------------------------------------- #
# AR Window Worker -- operates on the cached 128 Hz signal
# --------------------------------------------------------------------------- #
class ARWindowWorker(QtCore.QThread):
    """
    Slides windows over the cached 128 Hz signal and computes the per-window
    R value with the exact same rule find_spindles_lfp uses:

        burg(order=8, demean=False)
        -> poles with imag > 0
        -> mask = 10 <= freq <= 15 Hz
        -> R = max(|pole| for poles in mask)  else  0.0
    ...
    def __init__(self, signal_128, view_start_s, view_end_s, parent=None):
        super().__init__(parent)
        self.signal_128 = signal_128
        self.view_start_s = max(0.0, float(view_start_s))
        self.view_end_s = float(view_end_s)
    ...
    def run(self):
        try:
            n128 = len(self.signal_128)
            win = int(AR_WINDOW_SEC * AR_TARGET_FS)  # 128 samples

            # Stride is in 128 Hz samples.
            stride = max(1, int(AR_STRIDE_SAMPLES))

            # Window starts whose CENTER falls inside the visible range.
            #   center_128 = start_128 + win/2
            #   center_time = center_128 / 128
            # So we want start_128 such that start_time and center_time are in
            # [view_start, view_end].
            start_center = int(self.view_start_s * AR_TARGET_FS - win / 2)
            end_center = int(self.view_end_s * AR_TARGET_FS - win / 2)

            # Clamp to valid window starts.
            lo = max(0, start_center)
            hi = min(n128 - win, end_center)
            if hi < lo:
                self.finished_ok.emit(
                    self.view_start_s, self.view_end_s, [], []
                )
                return

            # Align to the global stride grid (start at 0, step stride).
            # That way every view sees the exact same stride positions the
            # detector used when it fit the whole file.
            first = ((lo + stride - 1) // stride) * stride
            starts = np.arange(first, hi + 1, stride)

            n_windows = len(starts)
            r_vals = np.zeros(n_windows, dtype=np.float64)
            n_in_band = 0

            for k, s in enumerate(starts):
                if self._is_cancelled:
                    return
                window = self.signal_128[s : s + win]
                if len(window) < win:
                    break
                try:
            ...
            # Time axis: window CENTER in absolute seconds.
            t_vals = (starts + win / 2.0) / AR_TARGET_FS
...
# --------------------------------------------------------------------------- #
# Signal Loader -- precomputes signal_128 once
# --------------------------------------------------------------------------- #
class SignalLoader(QtCore.QThread):
    """
    Emits (raw_signal, filtered_signal, signal_128, time_128).

    signal_128 is the SAME array the detector fits AR on. Caching it here
    means every view slides windows over identical numbers, eliminating the
    per-view downsampling phase jitter.
    """

    finished_ok = QtCore.pyqtSignal(object, object, object, object)
    failed = QtCore.pyqtSignal(str)
    ...
    def run(self):
        ...
        try:
            ...
            # Display-only filtered signal (10-15 Hz, same as before).
            filtered = butter_bandpass_filter(raw, BP_LOW, BP_HIGH, FS).astype(
                np.float32
            )
            filtered = np.nan_to_num(filtered, nan=0.0, posinf=0.0, neginf=0.0)

            # ---- Detector pipeline: exact same stages find_spindles_lfp uses.
            ar_filtered = ar_bandpass_filter(
                raw, lowcut=AR_PRE_LOWCUT, highcut=AR_PRE_HIGHCUT, fs=FS
            )
            ar_filtered = _sanitize(ar_filtered)
            signal_128 = _sanitize(
                ar_downsampling(ar_filtered, FS, AR_TARGET_FS)
            )
            time_128 = np.arange(len(signal_128), dtype=np.float64) / AR_TARGET_FS

            vprint(
                f"[Load] raw len={len(raw)} std={raw.std():.3f} | "
                f"signal_128 len={len(signal_128)}"
            )
            self.finished_ok.emit(raw, filtered, signal_128, time_128)
        except Exception as e:
...
class SpindleViewer(QtWidgets.QMainWindow):
    def __init__(self, manifest_path, ar_csv_path):
        super().__init__()
        self.setWindowTitle("Spindle Alignment Inspector (AR only)")
    ...
    def init_ui(self):
        ...
        filter_layout.addWidget(QtWidgets.QLabel("File:"))
        ...
        # Navigation
        nav_box = QtWidgets.QGroupBox("AR Detection Navigation")
        ...
        self.p_raw.setLabel("left", "Amplitude (Raw)", units="uV")
        ...
        self.curve_r = self.p_r.plot(
            pen=pg.mkPen(color="#ffab40", width=2), connect="finite"
        )
    ...
    def load_data(self):
        if not MODULES_AVAILABLE:
            self.details_panel.setText(
                f"Could not import ephys_preprocessing modules:\n{_IMPORT_ERROR}"
            )
            return
        if not os.path.exists(self.manifest_path):
            self.details_panel.setText(f"Manifest not found: {self.manifest_path}")
            return
        ...
        # AR events CSV
        if os.path.exists(self.ar_csv_path):
            self.ar_df = pd.read_csv(self.ar_csv_path)
            self.ar_df, _ = resolve_interval_columns(self.ar_df, FS)
            if "Rat" in self.ar_df.columns:
                self.ar_df["Rat"] = pd.to_numeric(
                    self.ar_df["Rat"], errors="coerce"
                ).fillna(-1).astype(int)
            ...
            if "File" in self.ar_df.columns:
                self.ar_df["File"] = self.ar_df["File"].apply(
                    lambda p: Path(str(p)).name
                )
    ...
    def on_rat_changed(self, index):
        if index < 0 or self.manifest_df is None:
            return
        rat = self.combo_rat.currentData()
        sub = self.manifest_df[self.manifest_df["Rat"] == rat]
    ...
    def on_file_selected(self, index):
        ...
        if not os.path.exists(data_path):
            self.details_panel.setText(f"File missing: {data_path}")
            return
    ...
    def _on_signal_load_failed(self, msg):
        QtWidgets.QApplication.restoreOverrideCursor()
        self.combo_files.setEnabled(True)
        self.status_bar.clearMessage()
        self.details_panel.setText(f"Signal error: {msg}")

    def _on_signal_loaded(self, raw, filtered, signal_128, time_128):
        QtWidgets.QApplication.restoreOverrideCursor()
        self.combo_files.setEnabled(True)
        self.status_bar.clearMessage()
        ...
        self.signal_128 = signal_128
        self.time_vector = np.arange(len(raw), dtype=np.float32) / FS
        ...
        self.curve_raw.setData(self.time_vector, raw)
        self.curve_filt.setData(self.time_vector, filtered)
        self.curve_r.clear()
        ...
        self.lbl_ar_tracker.setText(
            f"AR: {max(0, self.current_ar_idx + 1)} / "
        ...
        if self.current_ar_idx >= 0:
            ...
            self.details_panel.setText(
                f"File: {file_name} | Length: {self.data_duration:.1f}s | "
                f"No AR events."
            )
    ...
    def _start_window_computation(self):
        if not MODULES_AVAILABLE or self.signal_128 is None:
            return
        ...
        self._ar_signal_thread = ARWindowWorker(
            self.signal_128, start_s, end_s, parent=self
        )
    ...
    def _on_window_computation_ready(
        ...
        # Detector's 0 means "no in-band pole" -- show as gap in the trace.
        r_plot = np.where(r_arr > 0.0, r_arr, np.nan)

        self.curve_r.setData(t_arr, r_plot)
    ...
    def _on_window_computation_failed(self, msg):
        self.status_bar.showMessage(f"AR failed: {msg}", 12000)
    ...
    def _centered_view_range(self, start_s, end_s):
        ...
        if vs < 0.0:
            vs = 0.0
            ve = min(VIEW_WINDOW_SEC, self.data_duration)
        if ve > self.data_duration:
            ve = self.data_duration
            vs = max(0.0, ve - VIEW_WINDOW_SEC)
        return vs, ve
    ...
    def jump_to_ar_event(self):
        ...
        vs, ve = self._centered_view_range(start_s, end_s)
        self.p_raw.setXRange(vs, ve, padding=0)
...
def main():
    ...
    if VERBOSE:
        print("=" * 70)
        print("Spindle Alignment Inspector — AR only")
        ...
        print(f"AR_ORDER={AR_ORDER}  AR_WINDOW_SEC={AR_WINDOW_SEC}  "
              f"AR_STRIDE_SAMPLES={AR_STRIDE_SAMPLES}")
        print(f"AR_BAND={AR_SPINDLE_BAND}  "
              f"AR_PRE_FILTER=({AR_PRE_LOWCUT}, {AR_PRE_HIGHCUT})")
        print("=" * 70)
    ...
    palette.setColor(QtGui.QPalette.Highlight, QtGui.QColor(0, 188, 212))
    palette.setColor(QtGui.QPalette.HighlightedText, QtCore.Qt.black)
    app.setPalette(palette)

    viewer = SpindleViewer(args.manifest, args.ar_csv)
    viewer.show()
    sys.exit(app.exec_())
