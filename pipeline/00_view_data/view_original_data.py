...
for rat in [rats[2]]:
    ...
    for region in [regions[0]]:
        ...
        for channel in channels:
            ...
            matched_dir = fstore.find_path_by_suffix(path_recording, trial_suffix, search_dir=True)

            pattern = re.compile(rf"(?<![A-Z0-9])(?:CH)?{channel}\.continuous$")
            str_matched_file = fstore.find_path_by_suffix(matched_dir, pattern, search_dir=False, use_regex=True)
            # read recording
