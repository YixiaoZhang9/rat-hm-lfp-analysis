...
dir_R1_4_Data = os.path.join(dir_base,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_OpenEphysRecordings_R1-4/')
dir_results_base = '/media/yixiao/GL14_RAT_FA/'
dir_R1_4_results = os.path.join(dir_results_base,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis_New/R1-4/')
path_csv_overview_StudyDay = os.path.join(dir_R1_4_results,'Rat_HM_Ephys_TD_R1_4_Overview_StudyDay.csv')
path_csv_SelectedChannel = os.path.join(dir_R1_4_results, 'Rat_HM_Ephys_TD_R1_8_SelectedChannel.csv')
path_csv_postfix_trial = os.path.join(dir_R1_4_results,'Rat_HM_Ephys_TD_R1_4_Suffix_Trial.csv')
date_prefix = 'Rat_HM_Ephys_TD_R1-4_'
rats = np.arange(1,5)


dates_all = ['20220923','20220926','20220927','20220929','20221003','20221006','20221010',
             '20221012','20221013','20221014','20221018','20221021','20221024','20221025','20221026',
             '20221028','20221031','20221101','20221103','20221110','20221111', '20221115', '20221116',
        ...
        for date in dates_all:
            ...
            if date in dates_special_handels1:
                for period in sleep_period:
                    matched_dir = fstore.find_path_by_suffix(path_recording, period, search_dir=True)

                    # Edge case: 20220923 presleep contains two .continuous files
                    if date == '20220923' and period == 'presleep':
                        suffixes = ['', '_2']
                        for i, sfx in enumerate(suffixes):
                            pattern = re.compile(rf"(?<![A-Z0-9])(?:CH)?{channel}{sfx}.continuous$")
                            recording = fstore.read_recording(matched_dir, pattern)
                            save_dir = os.path.join(region_result_dir, period)
                            os.makedirs(save_dir, exist_ok=True)
                            save_name = f"chan{channel}{chr(97 + i)}.mat"  # e.g., _a, _b
                            savemat(os.path.join(save_dir, save_name), recording)
                    else:
                        fstore.save_sleep_recording(path_recording, period, channel, region_result_dir, period)
            ...
            # Case 2: Normal segmented recordings with trial suffixes
                ...
                for trial in trial_suffixes:
                    ...
                    if (date == '20220929' and trial == '_6') or (date == '20221006' and trial == '_4'):
                        matched_dir = fstore.find_path_by_suffix(path_recording, trial, search_dir=True)
                        suffixes = ['', '_2']
                        for i, sfx in enumerate(suffixes):
                            pattern = re.compile(rf"(?<![A-Z0-9])(?:CH)?{channel}{sfx}.continuous$")
                            recording = fstore.read_recording(matched_dir, pattern)
                            save_dir = os.path.join(region_result_dir, period)
                            os.makedirs(save_dir, exist_ok=True)
                            save_name = f"chan{channel}{trial}{chr(97 + i)}.mat"  # e.g., _a, _b
                            savemat(os.path.join(save_dir, save_name), recording)
