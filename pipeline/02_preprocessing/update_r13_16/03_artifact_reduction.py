...
import sys
#from Signal_Plot_pyqt import SignalPlotViewer
...
dir_R13_16_filteredData = os.path.join(dir_base1,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R13-16/Preprec_withartifacts')
...
dir_R13_16_PreprocessedData = os.path.join(dir_base1, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R13-16/PreprocessedData')
...
for rat in rats :  #for rat in [rats[0]]   for rat in rats
    ...
    for region in regions: # for region in [regions[0]] for region in regions
        ...
        dir_R13_16_filteredData_perday = os.path.join(dir_R13_16_filteredData, region, str(rat))
        folders_SD = [name for name in os.listdir(dir_R13_16_filteredData_perday)
                      if os.path.isdir(os.path.join(dir_R13_16_filteredData_perday, name))]

        for studyday in folders_SD: #for studyday in [folders_SD[0]] for studyday in folders_SD

            for sleep_period in sleep_periods:  #for sleep_period in sleep_periods

                # find all the trial recordings (suffix = ".mat")
                dir_R13_16_RawData_pertrial = os.path.join(dir_R13_16_filteredData_perday, studyday, sleep_period)
                All_files = []
                ...
                for file in All_files:
                    ...
                    save_dir = os.path.join(dir_R13_16_PreprocessedData, region,str(rat),studyday,sleep_period)
