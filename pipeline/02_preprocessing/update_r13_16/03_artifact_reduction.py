...
dir_R13_16_filteredData = os.path.join(
...
dir_R13_16_PreprocessedData = os.path.join(
...
for rat in rats:  # for rat in [rats[0]]   for rat in rats
    for region in regions:  # for region in [regions[0]] for region in regions
        dir_R13_16_filteredData_perday = os.path.join(
            dir_R13_16_filteredData, region, str(rat)
        )
        folders_SD = [
            name
            for name in os.listdir(dir_R13_16_filteredData_perday)
        ...
        for (
            ...
            for sleep_period in sleep_periods:  # for sleep_period in sleep_periods
                ...
                dir_R13_16_RawData_pertrial = os.path.join(
                    dir_R13_16_filteredData_perday, studyday, sleep_period
                )
                All_files = []
