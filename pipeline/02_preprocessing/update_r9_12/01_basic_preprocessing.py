...
dir_R9_12_RawData = os.path.join(dir_base, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R9-12/RawData')
dir_R9_12_Scoring = os.path.join(dir_base, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Sleepscoring_results_R9-12')

dir_R9_12_Preprec_withartifacts = os.path.join(dir_base, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R9-12/Preprec_withartifacts')
# this folder will store the preprocessed data
dir_R9_12_Scoring_withartifacts = os.path.join(dir_base, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R9-12/Scoring')
...
for rat in rats:  #for rat in [rats[0]]   rat in rats
    ...
    for region in regions: # for region in [regions[0]]  region in regions
        ...
        dir_R9_12_RawData_perday = os.path.join(dir_R9_12_RawData, region, str(rat))
        folders_SD = [name for name in os.listdir(dir_R9_12_RawData_perday)
        ...
        for studyday in folders_SD: #for studyday in [folders_SD[0]] studyday in folders_SD
            ...
            for sleep_period in sleep_periods:  #for sleep_period in sleep_periods
                ...
                dir_R9_12_RawData_pertrial = os.path.join(dir_R9_12_RawData_perday, studyday, sleep_period)
                matched_files = []
                ...
                dir_R9_12_Scoring_pertrial = os.path.join(dir_R9_12_Scoring, str(rat), studyday, sleep_period)
                matched_scoringfiles = []
                for root, dirs, files in os.walk(dir_R9_12_Scoring_pertrial):
                    for name in files:
                        if name.endswith(".mat"):
                ...
                for trial_name, files in grouped.items():  #for trial_name, files in [list(grouped.items())[0]]: for trial_name, files in grouped.items()
                    filtered_segments = []
                   ...
                   # Keep only the first 30 minutes of data
                    ...
                    save_dir = os.path.join(dir_R9_12_Preprec_withartifacts, region, str(rat), studyday, sleep_period)
                ...
                for scoring_name in matched_scoringfiles:
                    ...
                    save_dir_scoring = os.path.join(dir_R9_12_Scoring_withartifacts, str(rat), studyday, sleep_period)
