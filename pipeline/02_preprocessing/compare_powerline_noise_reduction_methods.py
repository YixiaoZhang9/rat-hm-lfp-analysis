...
from modules import Function_reduce_powerline_harmonics as powerline, ephys_preprocessing as processing
...
dir_R1_4_RawData = os.path.join(dir_base,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis_New/R1-4/RawData')

dir_R1_4_PreprocessedData = os.path.join(dir_base,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis_New/R1-4/PreprocessedData')
    ...
    for region in [regions[0]]: # for region in regions
        ...
        for studyday in [folders_SD[20]]: #for studyday in folders_SD
            ...
            for sleep_period in [sleep_period[1]]:  #for sleep_period in sleep_period
                ...
                for trial_name, files in [list(grouped.items())[0]]:  # for the testing and debuging
                    filtered_segments = []
                        ...
                        for method in methods:
                            try:
                                data_filtered = processing.powerline_filter(data_ds, ds_fs, Powerline_freq,
                                                                            Method = method, plot_data = True)

                                squeezed_data = data_ds.squeeze()
                                squeezed_filt_data = data_filtered.squeeze()

                                parr, freqs = powerline.calculate_parr(squeezed_data, squeezed_filt_data, ds_fs, powerline_freq=50
                                                                       , harmonic_n=9, bandwidth=2.0)
                                print(f"Method: {method} PARR: {parr:.2f} dB at frequencies {freqs}")

                                ppr, effective_bands = powerline.calculate_ppr_specific_band(squeezed_data, squeezed_filt_data,ds_fs,
                                    target_band=(0, 30))
                                print(f"target_band=(0, 30) PPR: {ppr:.2f} dB")

                                ppr, effective_bands = powerline.calculate_ppr_specific_band(squeezed_data, squeezed_filt_data, ds_fs,
                                    target_band=(100, 300),exclude_harmonics=[100, 150, 200, 250, 300],exclude_bw=0.5)
                    ...
                    if len(filtered_segments) == 1:
                        ...
                        for seg in filtered_segments[1:]:
                            combined_data = processing.smooth_transition(combined_data, seg, n=100)
