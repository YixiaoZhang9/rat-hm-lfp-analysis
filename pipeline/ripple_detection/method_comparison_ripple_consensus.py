...
def match_events(gt_df, pred_df, iou_th=0.5):
    ...
    for i, gt in gt_df.iterrows():
        ...
        while pred_ptr < len(pred_df) and pred_df.loc[pred_ptr, "ripple_end"] < gt_start:
            pred_ptr += 1
...
def get_voted_events(trial_methods_dict, selected_methods,anchor_order=["ripplenet", "cnn"], min_votes=2):
    """
        ...
        for studyday in os.listdir(dir_data):
            ...
            for sleep_period in sleep_periods:
                ...
                for trial_name in matched_files:
                    ...
                    csv_threshold = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples.csv")
                    ...
                    detection_results[rat][studyday][sleep_period][trial_id] = trial_dict
    ...
    for studyday in detection_results[rat]:
        for sleep_period in detection_results[rat][studyday]:
            for trial_id in detection_results[rat][studyday][sleep_period]:
                ...
                methods = detection_results[rat][studyday][sleep_period][trial_id]["methods"]
...
for rat in sorted(event_eval_df["rat"].unique()):
    ...
    summary = (subset.groupby("method")[["tp", "fp", "fn", "precision", "recall", "f1"]]
        .mean()
        .reset_index()
    )

    summary[["tp", "fp", "fn"]] = summary[["tp", "fp", "fn"]].round(1)
    summary[["precision", "recall", "f1"]] = summary[["precision", "recall", "f1"]].round(3)
    ...
    summary["sort_order"] = summary["method"].apply(lambda x: comparison_order.get(x, 999))
    ...
    available_methods = [
        ...
        if m in matrix.index or m in matrix.columns
    ]
