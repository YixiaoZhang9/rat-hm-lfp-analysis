...
dir_R9_12_Data = os.path.join(dir_base1, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R9-12/PreprocessedData')
dir_R9_12_Scoring = os.path.join(dir_base1, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R9-12/Scoring')

rats = np.arange(11,13)
regions = ['HPC','PL','RSC']
...
for rat in rats:  #rat in [rats[3]]
    ...
    for region in [regions[0]]:  # e.g. 'HPC'
        ...
        folders_SD = [name for name in os.listdir(dir_R9_12_Data_perday)
                      if os.path.isdir(os.path.join(dir_R9_12_Data_perday, name))]

        for studyday in folders_SD:
            print(f"\n===== Processing studyday: {studyday} =====")
            threshold_perday = []
            for sleep_period in sleep_periods:
                dir_R9_12_Data_pertrial = os.path.join(dir_R9_12_Data_perday, studyday, sleep_period)
                if not os.path.exists(dir_R9_12_Data_pertrial):
                    print(f'path : {dir_R9_12_Data_pertrial} does not exist')

                matched_files = []
                for root, _, files in os.walk(dir_R9_12_Data_pertrial):
                    for name in files:
                ...
                path_scoring = os.path.join(dir_R9_12_Scoring, str(rat), str(studyday), sleep_period)
...
total_df.to_csv('All_Rat11-12_Ripple_Features_17features.csv', index=False)
