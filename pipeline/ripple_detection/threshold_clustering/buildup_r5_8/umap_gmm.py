...
# ======================= Load Data ==========================
total_df = pd.read_csv("../../../../results/All_Rat5-8_Ripple_Features_17features_test.csv")

features = ['area','entropy','TW','FW','max_f','duration_ms','peak_env','rms_env',
            'relative_rise_time','relative_fall_time','env_skewness',
            'peak_energy_fraction','peak_to_trough','num_peaks',
            'zero_crossing_rate','good_cycle_ratio','Fuzz_value']
...
dir_R1_4_Data = os.path.join(dir_base1,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis_New/R5-8/PreprocessedData')
fs = 1000
window_ms = 500
...
for cluster_id in sorted(total_df['cluster_label'].unique()):
    ...
    for idx, row in samples.iterrows():
        ...
        plt.title(f"Rat{rat}-{region}-{studyday}-{sleep_period}-{trial_name}-{ripple_index}\nCluster {cluster_id}")
        plt.xlabel("Time (s)")
...
ax.scatter(total_df['UMAP1'], total_df['UMAP2'], total_df['UMAP3'],
