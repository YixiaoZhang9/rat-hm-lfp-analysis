...
for rat in rats:  #rat in [rats[3]]
    ...
    for region in [regions[0]]:  # e.g. 'HPC'
        ...
        folders_SD = [name for name in os.listdir(dir_R1_4_Data_perday)
        ...
        for studyday in folders_SD:
            ...
            for sleep_period in sleep_periods:
                dir_R1_4_Data_pertrial = os.path.join(dir_R1_4_Data_perday, studyday, sleep_period)
                    ...
                    for bout_idx, (start_sample, end_sample) in enumerate(NREM_bouts):
                        ...
                        ripple_karlsson = np.array(
                            list(zip(
                                ...
                                (Results_Threshold["PeakIndex"] + start_sample).astype(int),
                        ...
                        ripple_envelop_pertrial.extend(Results_Threshold["ripple_envelop"])
...
# ====================================== stockwell transform and related features=================================
                        ...
                        ripple_segment_reshaped = ripple_segment.reshape(1, 1, len(ripple_segment))
                        # ===  Stockwell time frequency transform ===
                        tfr_results = tfr_array_stockwell(ripple_segment_reshaped, fs, fmin=100, fmax=250, width=1.0)
                        power = np.abs(tfr_results[0]).squeeze()  # (n_freqs, n_times)
                        freqs = tfr_results[2]

                        ripple_time_indices = np.arange(seg_start, seg_end)
                        ripple_mask_time = (ripple_time_indices >= start_idx) & (ripple_time_indices <= end_idx)
                        ...
                        if f_plot:
                            ...
                            axs[0].axvline(start_time, color='r', linestyle='--', linewidth=2)
                            axs[0].axvline(end_time, color='r', linestyle='--', linewidth=2)
                            axs[0].set_ylabel("Amplitude")
                            axs[0].set_title(f"Ripple {i + 1} ±500 ms around peak")


                            # Stockwell tf  + ROI
                            im = axs[1].pcolormesh(t, freqs, 10 * np.log10(power + 1e-12),
                            ...
                            # add ROI contour
                            axs[1].contour(t, freqs, roi_mask, levels=[0.5], colors='white', linewidths=1.5)
...
# ============================================ time domain features ===========================================
                        ...
                        envelop_features = Ripple_features.ripple_envelop_features(ripple_envelop, ripple_segment, ripple_segment_filt, fs, rel_start_ind, rel_end_ind, plot = 0)

                        time_domain_features = Ripple_features.ripple_time_domain_features(ripple_segment, ripple_segment_filt, fs, rel_start_ind, rel_end_ind, plot = 0)

# ============================================ other features ===========================================
                        # fuzzy entropy
                        Fuzz,ps1,ps2 = EH.FuzzEn(ripple_data, m=2, tau=1, r=(0.2, 2.0), Fx='default')
                        Fuzz_value =  Fuzz[1]
                        ...
                        ripple_features.append({
                            ...
                            'relative_rise_time': envelop_features['relative_rise_time'],
                            'relative_fall_time': envelop_features['relative_fall_time'],
                            'env_skewness': envelop_features['env_skewness'],
                            'peak_energy_fraction': envelop_features['peak_energy_fraction'],
                            'peak_to_trough':time_domain_features['peak_to_trough'],
                            'num_peaks': time_domain_features['num_peaks'],
                            'zero_crossing_rate': time_domain_features['zero_crossing_rate'],
                            'good_cycle_ratio': time_domain_features['good_cycle_ratio'],
...
total_df.to_csv('All_Rat1-4_Ripple_Features_17features_test_3.5SD_0.5.csv', index=False)
print(f" Ripple features saved! Total {len(total_df)} ripples.")
                    ...
                    #     ripple_data = data[seg_start:seg_end]
                    #
                    #     # reshape to (1, 1, n_times) for tfr_array_stockwell
                    #     ripple_data = ripple_data.reshape(1, 1, len(ripple_data))
                    #
                    #     tfr_results = tfr_array_stockwell(ripple_data, fs, fmin=80, fmax=250,width=2.0)
                    #     power = tfr_results[0].squeeze() # (1,n_freqs,n_times)
                    #     freqs = tfr_results[2]
                    #
                    #     t = (np.arange(seg_start, seg_end) - peak_idx) / fs
                    #     start_time = (start_idx - peak_idx) / fs
                    ...
                    #     fig, axs = plt.subplots(2, 1, figsize=(10, 6), sharex=True)
                    #
                    #     axs[0].plot(t, ripple_data.squeeze(), 'k')
                    #     axs[0].axvline(0, color='r', linestyle='--', linewidth=2)
                    #     axs[0].axvline(start_time, color='r', linestyle='--', linewidth=2)
                    #     axs[0].axvline(end_time, color='r', linestyle='--', linewidth=2)
                    #     axs[0].set_ylabel("Amplitude")
                    #     axs[0].set_title(f"The {i+1}th  Ripple ±500ms around peak")
                    #
                    #
                    #     im = axs[1].pcolormesh(t, freqs, 10 * np.log10(power + 1e-12),
                    #                            shading='gouraud', cmap='jet')
                    ...
                    # window_ms = 250  # 250ms on both left and right
                    # half_window_samples = int(fs * window_ms / 1000)
                    # # plot the detected ripples and time-frequency analysis
                    # for i, ripple in enumerate(ripple_pertrial):
                    #     start_idx, peak_idx, end_idx = ripple
                    #
                    #     seg_start = max(0, peak_idx - half_window_samples)
                    ...
                    #
                    #
                    #     axs[0].plot(t, ripple_data, 'k')
                    #     axs[0].axvline(0, color='r', linestyle='--', linewidth=2)
                    #     axs[0].axvline(start_time, color='r', linestyle='--', linewidth=2)
                    #     axs[0].axvline(end_time, color='r', linestyle='--', linewidth=2)
                    #     axs[0].set_ylabel("Amplitude")
                    #     axs[0].set_title("Ripple ±250ms around peak")
                    #
                    #
                    #     f, tt, Sxx = spectrogram(ripple_data, fs=fs, nperseg=int(0.05 * fs), noverlap=int(0.025 * fs))
                    #     tt_centered = tt + t[0]
                    #     im = axs[1].pcolormesh(tt_centered, f, 10 * np.log10(Sxx), shading='gouraud', cmap='jet')
                    #     axs[1].axvline(0, color='r', linestyle='--', linewidth=2)
                    #     axs[1].set_ylim(80, 250)
                    #     axs[1].set_xlabel("Time (s)")
                    #     axs[1].set_ylabel("Frequency (Hz)")
