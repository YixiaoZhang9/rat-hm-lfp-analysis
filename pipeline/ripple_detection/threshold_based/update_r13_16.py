...
dir_R13_16_Data = os.path.join(dir_base1, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R13-16/PreprocessedData')
dir_R13_16_Scoring = os.path.join(dir_base1, 'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R13-16/Scoring')
...
#%%
# detect ripples
for rat in [rats[3]]:  #rat in [rats[3]]
    rat_str = f'rat{rat}'
    print(f"\n===== Processing {rat_str} =====")
    threshold_perrat = []
    ...
    for region in [regions[0]]:  # e.g. 'HPC'
        ...
        # folders_SD = [name for name in os.listdir(dir_R13_16_Data_perday)
        #               if os.path.isdir(os.path.join(dir_R13_16_Data_perday, name))]

        folders_SD = sorted(
            [name for name in os.listdir(dir_R13_16_Data_perday)
             if os.path.isdir(os.path.join(dir_R13_16_Data_perday, name))],
            key=lambda x: int(x)
        )

        for studyday in [folders_SD[24]]:

            # skip the empty filder_SD
            if not os.listdir(os.path.join(dir_R13_16_Data_perday, studyday)):
                NREM_duration_across_time[rat_str]['presleep'].append(np.nan)
                NREM_duration_across_time[rat_str]['postsleep'].append(np.nan)
                Ripple_count_across_time[rat_str]['presleep'].append(np.nan)
                Ripple_count_across_time[rat_str]['postsleep'].append(np.nan)
                threshold_perday = np.nan
                threshold_perrat.append(threshold_perday)
                continue

            threshold_perday = []
            for sleep_period in [sleep_periods[1]]:
                dir_R13_16_Data_pertrial = os.path.join(dir_R13_16_Data_perday, studyday, sleep_period)
                if not os.path.exists(dir_R13_16_Data_pertrial):
                    print(f'path : {dir_R13_16_Data_pertrial} does not exist')

                matched_files = []
                for root, _, files in os.walk(dir_R13_16_Data_pertrial):
                    for name in files:
                ...
                path_scoring = os.path.join(dir_R13_16_Scoring, str(rat), str(studyday), sleep_period)
                ...
                for trial_name in [matched_files[0]]:
                    ...
                    for bout_idx, (start_sample, end_sample) in enumerate(NREM_bouts):
                        ...
                        Results_Threshold = find_ripples_karlsson(bout_data, fs, f_plot = 0)

                        ripple_karlsson = np.array(
                            list(zip(
                                (Results_Threshold["StartIndex"] + start_sample).astype(int),
                                (Results_Threshold["EndIndex"]  + start_sample).astype(int)
                            ))
                        )

                        ripple_count_per_period += len(ripple_karlsson)
                        ripple_pertrial.extend(ripple_karlsson)
                        thresholds_per_trial.append(Results_Threshold["thresh_envelope"])
                ...
                Ripple_count_across_time[rat_str][sleep_period].append(ripple_count_per_period)
...
for rat, rat_data in Threshold_across_time.items():
    print(f"length of rat_data {rat_data}")
    mean_thresh_perday[rat] = []
    std_thresh_perday[rat] = []
    for day_data in rat_data: # day_data = [presleep_thresh, postsleep_thresh]

        # -------- Handle empty day --------
        if day_data is np.nan or day_data is None:
            mean_thresh_perday[rat].append(np.nan)
            std_thresh_perday[rat].append(np.nan)
            continue

        all_bout_means = []

        for trial_data in day_data:
            if trial_data is np.nan or trial_data is None:
                continue
            if isinstance(trial_data, (float, np.float64)):
                all_bout_means.append(trial_data)
                continue
            if len(trial_data) > 0:
                bout_means = [np.mean(bout) for bout in trial_data]
                all_bout_means.extend(bout_means)

        if len(all_bout_means) == 0:
            mean_thresh_perday[rat].append(np.nan)
            std_thresh_perday[rat].append(np.nan)
        else:
            mean_thresh_perday[rat].append(np.mean(all_bout_means))
            std_thresh_perday[rat].append(np.std(all_bout_means))
...
plt.title('Ripple Threshold Across Days (Rat 9-12)', fontsize=26)
...
#                         Results_Threshold_m2 = find_ripples_karlsson(bout_data, fs, f_plot = 0,threshold = threhold_day)
#
#                         ripple_karlsson = np.array(
#                             list(zip(
#                                 (Results_Threshold_m2["StartIndex"] * fs + start_sample).astype(int),
#                                 (Results_Threshold_m2["EndIndex"] * fs + start_sample).astype(int)
#                             ))
#                         )
#
...
#                         print("end")
#
#                 Ripple_count_across_time_m2[rat_str][sleep_period].append(ripple_count_per_period_m2)
...
for rat in rats:
    ...
    plt.bar(days, postsleep_vals_nrem, bottom=presleep_vals_nrem, label='Postsleep', color='lightcoral', alpha=0.8)
    ...
    plt.bar(days, ripple_count_post, bottom=ripple_count_pre, label='Postsleep', color='lightcoral', alpha=0.8)
    ...
    # ---------- Plot 3: Ripple rate ----------
    Total_Ripple_count_across_time = (np.array(Ripple_count_across_time[rat_str]['presleep'])) + np.array(Ripple_count_across_time[rat_str]['postsleep'])
...
for rat in rats:
    ...
    ripple_count_perday[rat_str] = [a + b for a, b in zip(
    Ripple_count_across_time[rat_str]['presleep'],
    Ripple_count_across_time[rat_str]['postsleep'])]

    ripple_count_perday_m2 [rat_str] = [a + b for a, b in zip(
    ...
    ax1.plot(days, thresh_day_m2, 's--', color='green', label='Threshold (Method 2 avg)')
    ...
    ax2 = ax1.twinx()
    width = 0.35
    ax2.bar(days - width / 2, ripple_day, width=width, color='blue', alpha=0.7, label='Ripple Count (Method 1)')
    ax2.bar(days + width / 2, ripple_day_m2, width=width, color='green', alpha=0.7, label='Ripple Count (Method 2)')
