...
from modules.threshold_ripple_detection import find_ripples_karlsson, find_bouts, filter_lfp,find_ripples_karlsson_modified,estimate_noise_mirror_threshold
...
for rat in rats:  #rat in [rats[3]]
    ...
    for region in [regions[0]]:  # e.g. 'HPC'
        ...
        for studyday in folders_SD:
            ...
            threshold_perday_z = estimate_noise_mirror_threshold(zscored_envelope, bins=500, percentile=0.9999, plot=0)

            threshold_perday = threshold_perday_z * np.std(envelope_per_day) + np.mean(envelope_per_day)
        ...
        for studyday in folders_SD:
            ...
            for sleep_period in sleep_periods:
                ...
                for trial_name in matched_files:
                    ...
                    for bout_idx, (start_sample, end_sample) in enumerate(NREM_bouts):
                        bout_data = data[start_sample:end_sample]
                        Results_Threshold = find_ripples_karlsson_modified(bout_data, fs, f_plot=0)
...
for rat in rats:
    ...
    plt.bar(days, postsleep_vals_ripples, bottom=presleep_vals_ripples, label='Postsleep', color='lightcoral', alpha=0.8)
