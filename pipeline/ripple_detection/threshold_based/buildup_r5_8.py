...
for rat in rats:  #rat in [rats[3]]
    ...
    for region in [regions[0]]:  # e.g. 'HPC'
        dir_R1_4_Data_perday = os.path.join(dir_R5_8_Data, region, str(rat))
                ...
                for trial_name in matched_files:
                    ...
                    for bout_idx, (start_sample, end_sample) in enumerate(NREM_bouts):
                        ...
                        ripple_karlsson = np.array(
                            list(zip(
                                (Results_Threshold["StartIndex"] * fs + start_sample).astype(int),
                                (Results_Threshold["EndIndex"] * fs + start_sample).astype(int)
                            ))
                        )
            ...
            for sleep_period in sleep_periods:
                ...
                for trial_name in matched_files:
                    ...
                    for bout_idx, (start_sample, end_sample) in enumerate(NREM_bouts):
                        ...
                        ripple_karlsson = np.array(
                            list(zip(
                                (Results_Threshold_m2["StartIndex"] + start_sample).astype(int),
                                (Results_Threshold_m2["EndIndex"] + start_sample).astype(int)
                            ))
                        )
