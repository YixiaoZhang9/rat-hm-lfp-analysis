...
model_path = ripplenet_model_dir /"model_training"/"retrained_model.pkl"
...
model = keras.models.load_model(ripplenet_model_dir /"model_training"/ "retrained_model.h5",compile=False)
model.summary()
...
def ripplenet_predict(lfp_segment, fs, model):
    ...
    for i in range(n_shifts):
        ...
        X_pad = np.concatenate(
            (np.zeros((1, i * shift, 1), dtype=np.float32), X),
            axis=1
        )
        ...
        if remainder != 0:
            ...
            X_pad = np.concatenate(
                (X_pad, np.zeros((1, pad_len, 1), dtype=np.float32)),
                axis=1
            )
...
def detect_ripples_from_prediction(pred, fs, threshold, distance, width, offset = 0):
    ...
    Y_pred_binary,predicted_peaks = get_binary_predictions(pred.flatten(), threshold, distance, width)
...
dir_output = os.path.join(dir_base1,'Rat_HM_Ephys_TD/Rat_HM_Ephys_TD_Analysis/R13-16/Ripple_detection_results')
...
for rat in rats:
    ...
    for region in [regions[0]]:
        ...
        folders_SD = [d for d in os.listdir(dir_R13_16_Data_perday)
                      if os.path.isdir(os.path.join(dir_R13_16_Data_perday, d))]
        ...
        for studyday in folders_SD:
            ...
            for sleep_period in sleep_periods:
                ...
                path_scoring = os.path.join(dir_R13_16_Scoring, str(rat), studyday, sleep_period)
                ...
                for root, _, files in os.walk(dir_trial):
                    for name in files:
                        ...
                        if len(scoring_files) == 1:
                            scoring = loadmat(os.path.join(path_scoring, scoring_files[0]))['states'].squeeze()
                        else:
                            ...
                            for f in scoring_files:
                                if f"_{suffix}_" in f:
                                    scoring = loadmat(os.path.join(path_scoring, f))['states'].squeeze()
                                    break
                        ...
                        for b_start, b_end in bouts:
                            ...
                            # Convert to events
                            events = detect_ripples_from_prediction(predicted_ripples, fs, threshold, distance, width, offset=b_start)
                        ...
                        save_path = os.path.join(output_dir, f"{trial_id}_hippocampal_ripples_ripplenet2.csv")
