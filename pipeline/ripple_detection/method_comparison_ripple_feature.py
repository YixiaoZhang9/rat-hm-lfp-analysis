...
def compute_wavelet_frequency(signal, onset, offset, peak_idx, fs):
    ...
    if ripple_spec.size == 0: return np.nan
    ...
    for studyday in detection_results[rat]:
        for sleep_period in detection_results[rat][studyday]:
            for trial_id in detection_results[rat][studyday][sleep_period]:
                ...
                pre_lfp = detection_results[rat][studyday][sleep_period][trial_id]["pre_signal"]

                # methods duration
                for method, pred_df in detection_results[rat][studyday][sleep_period][trial_id]["methods"].items():
...
sns.violinplot(
    data=df_feat[df_feat["feature"] == "duration"],
    x="method",
    y="value",
    cut=0
)
