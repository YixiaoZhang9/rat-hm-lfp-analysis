...
dir_R5_8_Ripple = os.path.join(dir_data_root,'Ripple_detection_results')
    ...
    for region in ['HPC']:
        ...
        for studyday in os.listdir(dir_data):
            ...
            for sleep_period in sleep_periods:
                ...
                dir_ripple = os.path.join(dir_R5_8_Ripple, str(rat), studyday, sleep_period)
                ...
                for trial_name in matched_files:
                    ...
                    # --- CSV paths ---
                    csv_threshold = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_threshold.csv")
                    csv_threshold2 = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_threshold2.csv")
                    csv_cnn = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_cnn.csv")
                    csv_ripplenet = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_ripplenet.csv")
...
# remove the data on 20221101 in rat7 (super noisy signal)
rat_noisy_day = [
    '20221107','20221108','20221109','20221111',
    '20221117','20221118','20221121','20221123',
    '20221124','20221125','20221128','20221129',
    '20221205','20221206','20221207','20221209'
]

df = df[~(
    (df['rat'] == 'rat7') &
    (df['day'].isin(rat_noisy_day))
)]
...
df_long = pd.DataFrame(
    df_long,
    columns=['rat', 'condition', 'ripple_rate']
).dropna()
...
# ---- Find scoring file ----
dir_scoring = os.path.join(dir_R5_8_Scoring, rat, studyday, sleep_period)
scoring_files = [f for f in os.listdir(dir_scoring) if f.endswith(".mat")]
# matched sleep scoring files
if len(scoring_files) == 1:
    scoring = loadmat(os.path.join(dir_scoring, scoring_files[0]))['states'].squeeze()
...
# ---- Find ripple CSVs ----
dir_ripple = os.path.join(dir_R5_8_Ripple, rat, studyday, sleep_period)
csv_threshold = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_threshold.csv")
csv_threshold2 = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_threshold2.csv")
csv_threshold3 = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_threshold3.csv")
# csv_cnn = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_cnn2.csv")
# csv_ripplenet = os.path.join(dir_ripple, f"{trial_id}_hippocampal_ripples_ripplenet2.csv")

threshold_events = load_events(csv_threshold) if os.path.exists(csv_threshold) else []
threshold2_events = load_events(csv_threshold2) if os.path.exists(csv_threshold2) else []
threshold3_events = load_events(csv_threshold3) if os.path.exists(csv_threshold3) else []
