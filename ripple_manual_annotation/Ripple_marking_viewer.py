...
class RippleViewer(QWidget):
    ...
    def __init__(self, lfp, scoring, fs, events_dict):
        ...
        self.colors = {key: default_colors[i % len(default_colors)]
    ...
    def add_legends(self):
        ripple_legend = [Line2D([0], [0], color=self.colors[k], lw=3, label=k)
    ...
    def plot_ripples(self, start, end):
        ...
        for i, key in enumerate(self.annotator_keys):
            ...
            self._draw_bars(events, start/self.fs, end/self.fs, y_max + (0.05 + 0.07*i)*height, self.colors[key])
